...
Think of it as the ability to "lift" values into our monad.  For example, if we make η the list constructor `[ ]`, then lifting some value "a" would be literally `[a]`.  Your `a` now has the type `List a`, which slots into the needed `m a` from before.  We call that lifting function `return`.  It’s best to cast away any notion of `return` as a way of returning values from a function.  In Haskell, every time you see `return`, even at the end of do blocks, it’s just lifting a value into a monad.

Finally, the multiplication, `μ`, is a way of reducing the structure of nested monads.  Practically, if I have something like `[[a]]`, then applying ν to the list of lists would strip away one set of brackets and concat the inner arrays, yielding a longer `[a]`.  But notice, this corresponds precisely to `mconcat` in our Monad Typeclass!  Haskell calls the monad version of this function `join`.  Here is its type:
...
Sequencing monads together (chaining) is accomplished by those scary looking arrow things you’ve seen once or twice denoted
...
The first is pronounced “bind”, the second, “sequence”.  From bind’s type, it’s easy to see what it does:
...
Sequence can also be inferred from its type.  It takes the monad to the left, evaluates whatever’s in it, discards the result, then evaluates the monad to the right and returns it. Sounds complicated, but we also have the exact same thing built into nearly every C-like language: the comma operator.  In C, `f(), g();` calls f, throws away the result, then calls g and makes the result of the entire expression its return value.
...
The left and down arrows say that if I run `join` across this list, that it'll take away a pair of brackets.
...
You can think of the exponents next to the T as the number of brackets you have to `join` to reach some "minimal monad" for which no more extraneous structure can be removed.

The second is the unit triangle.  It says that we can take our monad, add "another set of brackets" with `η` on the left or right, then `join` (`μ`) it and get the same thing back.  This translates to Haskell as
...
main = lift3 Blog.generatePost Window.dimensions (constant "Monards") (constant post)